            ...
            <div className={styles.navigationBarRow}>
                {resource?.articleTypes?.map((articleType, index) => (
                    articleType.description !== 'Boletins' ? (
                        <Link key={index} className={styles.label} style={{ color: articleType.color }} to={`/propositions-by-type/${articleType.id}`} aria-label={`Ir para a página de listagem de ${articleType.description}`}>
