...
export const VotingListPage: FC<Props> = memo(function VotingListPage(props = {}) {
    ...
    const fetchArticles = async (page?: number) => {
        try {
            ...
            const articleType = resource?.articleTypes?.find((type) => type.codes == 'voting')
            if (articleType) setArticleType(articleType)
    ...
    const fetchTrendingArticles = async () => {
        try {
            const queryFilters: ArticleFilters = {
                typeId: resource?.articleTypes?.find((type) => type.codes == 'voting')?.id,
                itemsPerPage: 5
            };
    ...
    const handleFilterClick = () => {
        if (!!(content || startDate || endDate || (articleType && articleType.codes !== 'voting') || votingStartDate || votingEndDate || votingResult || votingLegislativeBody)) {
                    ...
                    articleList?.length > 0 ? (
                        <div className={styles.newslettersContainer}>
                            <div className={styles.newslettersLeftColumn}>
                                <TitleTopic titleViewStyle={styles.newslettersTitleView} label="Votações" color={articleList.at(0)!.type.color}/>
